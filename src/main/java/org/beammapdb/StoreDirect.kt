...
class StoreDirect(
...
): org.beammapdb.StoreDirectAbstract(
        file=file,
        volumeFactory=volumeFactory,
    ...
    override protected val volume: Volume = {
        volumeFactory.makeVolume(
                ...
                isReadOnly,
                fileLockWait,
                Math.max(org.beammapdb.CC.PAGE_SHIFT, DataIO.shift(allocateIncrement.toInt())),
    ...
    init{
        org.beammapdb.Utils.lock(structuralLock) {
            if (!volumeExistsAtStart) {
                //TODO crash resistance while file is being created
                //initialize values
                volume.ensureAvailable(org.beammapdb.CC.PAGE_SIZE)
    ...
    override protected fun getIndexVal(recid:Long):Long{
        if(CC.PARANOID) //should be ASSERT, but this method is accessed way too often
            org.beammapdb.Utils.assertReadLock(locks[recidToSegment(recid)])

        try {
            val offset = recidToOffset(recid)
            val indexVal = volume.getLong(offset);
            if(indexVal == 0L)
                throw org.beammapdb.DBException.GetVoid(recid)
    ...
    override protected fun setIndexVal(recid:Long, value:Long){
        if(CC.ASSERT)
            org.beammapdb.Utils.assertWriteLock(locks[recidToSegment(recid)])
    ...
    override protected fun allocateNewIndexPage():Long{
        ...
        if(CC.ASSERT && parity16Get(volume.getLong(pagePointerOffset))!=0L)
            throw org.beammapdb.DBException.DataCorruption("index pointer not empty")
    ...
    protected fun linkedRecordPut(output:ByteArray, size:Int):Long{
        ...
        var chunkOffset = org.beammapdb.Utils.lock(structuralLock) {
        ...
        while(remSize>0){
            ...
            chunkOffset = org.beammapdb.Utils.lock(structuralLock) {
    ...
    override protected fun longStackPut(masterLinkOffset:Long, value:Long, recursive:Boolean){
        ...
        if (CC.ASSERT && (masterLinkOffset <= 0 || masterLinkOffset > CC.PAGE_SIZE || masterLinkOffset % 8 != 0L))
            throw org.beammapdb.DBException.DataCorruption("wrong master link")
    ...
    override protected fun longStackTake(masterLinkOffset:Long, recursive:Boolean):Long {
        ...
        if(CC.ASSERT && pos<8L)
            throw org.beammapdb.DBException.DataCorruption("position too small")

        if(CC.ASSERT && volume.getLong(offset).ushr(48)<=pos)
            throw org.beammapdb.DBException.DataCorruption("position beyond chunk " + masterLinkOffset);
    ...
    override fun preallocate(): Long {
        assertNotClosed()
        val recid = org.beammapdb.Utils.lock(structuralLock) {
            allocateRecid()
        }

        org.beammapdb.Utils.lockWrite(locks[recidToSegment(recid)]) {
            if (CC.ASSERT) {
    ...
    override fun <R> get(recid: Long, serializer: Serializer<R>): R? {
        ...
        org.beammapdb.Utils.lockRead(locks[recidToSegment(recid)]) {
    ...
    override fun getBinaryLong(recid:Long, f: StoreBinaryGetLong): Long {
        ...
        org.beammapdb.Utils.lockRead(locks[recidToSegment(recid)]) {
            ...
            if (indexValFlagLinked(indexVal)) {
                val di = linkedRecordGet(indexVal)
                return f.get(org.beammapdb.DataInput2.ByteArray(di), di.size)
            }
            ...
            val di =
                    if (size >= 6)
                        ...
                        putLong(buf, 0, offset.ushr(8), sizeInt)
    ...
    override fun <R> put(record: R?, serializer: Serializer<R>): Long {
        ...
        org.beammapdb.Utils.lockRead(compactionLock) {
            ...
            Utils.lockWrite(locks[recidToSegment(recid)]) {
                ...
                if (di.pos == 0) {
                    ...
                    offset = getLong(di.buf, 0).ushr((7 - di.pos) * 8)
                } else {
    ...
    override fun compact() {
        org.beammapdb.Utils.lockWrite(compactionLock) {
            Utils.lockWriteAll(locks)
            try {
                Utils.lock(structuralLock) {
                    //TODO use file for compaction, if store is file based
                    val store2 = make(isThreadSafe = false, concShift = 0)
    ...
    override fun getAllRecids(): LongIterator {
        ...
        org.beammapdb.Utils.lockReadAll(locks)
        try {
            val maxRecid = maxRecid
            ...
            org.beammapdb.Utils.unlockReadAll(locks)
        }
    ...
    override fun verify(){
        ...
        try {
            ...
            val zeroIndexPageEnd = Math.min(org.beammapdb.CC.PAGE_SIZE, recidToOffset(maxRecid) + 8)
