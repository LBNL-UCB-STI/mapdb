...
class SortedTableMap<K,V>(
    ...
    abstract class Sink<K, V> : org.beammapdb.Pump.Sink<Pair<K, V>, SortedTableMap<K, V>>() {
    ...
    companion object {
        ...
        @JvmStatic fun <K, V> open(
            ...
            if (pageSize <= 0)
                throw org.beammapdb.DBException.DataCorruption("Wrong page size: " + pageSize)
            val volSliceSize = volume.sliceSize()
            if(volSliceSize>0 && volSliceSize<pageSize)
                throw org.beammapdb.DBException.WrongConfiguration("Slice Size of underlying Volume is too small.")
                ...
                override fun put(e: Pair<K, V>) {
                    if(oldKey!=null){
                        if(keySerializer.compare(oldKey,e.first)>=0)
                            throw org.beammapdb.DBException.NotSorted()
                    }
                    oldKey = e.first
                    pairs.add(e)
                    counter++
    ...
    init{
        ...
        if(volume.getUnsignedShort(2) != 0)
            throw org.beammapdb.DBException.NewMapDBFormat("SortedTableMap file was created with newer MapDB version")

        if(volume.getInt(4)!=0)
            throw org.beammapdb.DBException.NewMapDBFormat("SortedTableMap has some extra features, not supported in this version")
    }
    ...
    override val keys: NavigableSet<K> = org.beammapdb.BTreeMapJava.KeySet<K>(this as ConcurrentNavigableMapExtra<K, Any>, true)
