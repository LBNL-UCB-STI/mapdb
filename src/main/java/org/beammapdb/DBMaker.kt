...
object DBMaker{
    ...
    @JvmStatic fun memoryShardedHashSet(concurrency:Int): org.beammapdb.DB.HashSetMaker<*> {
        val db = DB(store = StoreOnHeap(), storeOpened = false, isThreadSafe = true)
        return org.beammapdb.DB.HashSetMaker<Any>(db, "map", _storeFactory = { i ->
            StoreDirect.make(isThreadSafe = false)
        })
            .layout(concurrency = concurrency, dirSize = 1.shl(org.beammapdb.CC.HTREEMAP_DIR_SHIFT), levels = CC.HTREEMAP_LEVELS)
    }

    @JvmStatic fun heapShardedHashSet(concurrency:Int): org.beammapdb.DB.HashSetMaker<*> {
        val db = DB(store = StoreOnHeap(), storeOpened = false, isThreadSafe = true)
        return org.beammapdb.DB.HashSetMaker<Any>(db, "map", _storeFactory = { i ->
    ...
    @JvmStatic fun memoryShardedHashMap(concurrency:Int): org.beammapdb.DB.HashMapMaker<*, *> {
        val db = DB(store = StoreOnHeap(), storeOpened = false, isThreadSafe = true)
        return org.beammapdb.DB.HashMapMaker<Any, Any>(db, "map", _storeFactory = { i ->
            StoreDirect.make(isThreadSafe = false)
        })
            .layout(concurrency = concurrency, dirSize = 1.shl(org.beammapdb.CC.HTREEMAP_DIR_SHIFT), levels = CC.HTREEMAP_LEVELS)
    }

    @JvmStatic fun heapShardedHashMap(concurrency:Int): org.beammapdb.DB.HashMapMaker<*, *> {
        val db = DB(store = StoreOnHeap(), storeOpened = false, isThreadSafe = true)
        return org.beammapdb.DB.HashMapMaker<Any, Any>(db, "map", _storeFactory = { i ->
            StoreOnHeap(isThreadSafe = false)
        })
    ...
    class Maker(
        ...
        private var _concurrencyScale: Int = 1.shl(org.beammapdb.CC.STORE_DIRECT_CONC_SHIFT)
...
//        }
        ...
        protected fun assertFile(){
            if((_storeType in arrayOf(StoreType.fileRaf, StoreType.fileMMap, StoreType.fileChannel)).not())
                throw org.beammapdb.DBException.WrongConfiguration("File related options are not allowed for in-memory store")
        }
        ...
        fun make(): DB {
            ...
            val concShift = DataIO.shift(org.beammapdb.DataIO.nextPowTwo(_concurrencyScale))
            ...
            var volfab = when(storeType2){
                ...
                StoreType.fileMMap -> org.beammapdb.volume.MappedFileVol.MappedFileFactory(_cleanerHack, _fileMmapPreclearDisable)
                null -> VolumeFactory.wrap(_customVolume!!, _volumeExist!!)
            }

            if(_readOnly && volfab!=null && volfab.handlesReadonly().not())
                volfab = org.beammapdb.volume.ReadOnlyVolumeFactory(volfab)

            var store = if(_storeType == StoreType.onheap){
                    if(_readOnly)
                        org.beammapdb.StoreReadOnlyWrapper(org.beammapdb.StoreOnHeap())
                    else
                        org.beammapdb.StoreOnHeap()
                }else {
                    storeOpened = volfab!!.exists(file)
                    if (_transactionEnable.not() || _readOnly) {
                        org.beammapdb.StoreDirect.Companion.make(file = file, volumeFactory = volfab!!,
                        ...
                        if(_checksumStoreEnable)
                            throw org.beammapdb.DBException.WrongConfiguration("Checksum is not supported with transaction enabled.")
                        org.beammapdb.StoreWAL.Companion.make(file = file, volumeFactory = volfab!!,
                                fileLockWait = _fileLockWait,
                                allocateIncrement = _allocateIncrement,
                                allocateStartSize = _allocateStartSize,
                                fileDeleteAfterClose = _fileDeleteAfterClose,
