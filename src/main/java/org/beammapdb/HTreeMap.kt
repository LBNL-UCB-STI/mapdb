...
class HTreeMap<K,V>(
        ...
        val dirShift: Int,
        val levels: Int,
        val stores: Array<org.beammapdb.Store>,
        ...
        val expireCreateQueues:Array<org.beammapdb.QueueLong>?,
        val expireUpdateQueues:Array<org.beammapdb.QueueLong>?,
        val expireGetQueues:Array<org.beammapdb.QueueLong>?,
...
) : ConcurrentMap<K, V>, ConcurrencyAware, org.beammapdb.MapExtra<K, V>, org.beammapdb.Verifiable, Closeable {
    ...
    companion object{
        ...
        fun <K,V> make(
                ...
                stores:Array<org.beammapdb.Store> = Array(1.shl(concShift), { org.beammapdb.StoreTrivial() }),
                indexTrees: Array<MutableLongLongMap> = Array(1.shl(concShift), { i-> org.beammapdb.IndexTreeLongLongMap.Companion.make(stores[i], levels = levels, dirShift = dirShift) }),
                ...
                expireCreateQueues:Array<org.beammapdb.QueueLong>? = if(expireCreateTTL<=0L) null else Array(stores.size, { i-> org.beammapdb.QueueLong.Companion.make(store = stores[i]) }),
                expireUpdateQueues:Array<org.beammapdb.QueueLong>? = if(expireUpdateTTL<=0L) null else Array(stores.size, { i-> org.beammapdb.QueueLong.Companion.make(store = stores[i]) }),
                expireGetQueues:Array<org.beammapdb.QueueLong>? = if(expireGetTTL<=0L) null else Array(stores.size, { i-> org.beammapdb.QueueLong.Companion.make(store = stores[i]) }),
    ...
    private val storesUniqueCount = org.beammapdb.Utils.identityCount(stores)

    protected val locks:Array<ReadWriteLock?> = Array(segmentCount, { org.beammapdb.Utils.newReadWriteLock(isThreadSafe) })
    ...
    init{
        ...
        if(org.beammapdb.BTreeMap.Companion.NO_VAL_SERIALIZER ==valueSerializer && hasValues)
        ...
        if(segmentCount*Math.pow(1.shl(dirShift).toDouble(),levels.toDouble()) > 2L*Integer.MAX_VALUE+1000){
            org.beammapdb.Utils.LOG.warning { "Wrong layout, segment+index is more than 32bits, performance degradation" }
        }
    }
     ...
     */
    private var checkHashAfterSerialization = stores.find { it is org.beammapdb.StoreOnHeap } == null
    ...
    private fun counter(segment:Int, ammount:Int){
        if(counterRecids==null)
            return
        if(CC.ASSERT)
            org.beammapdb.Utils.assertWriteLock(locks[segment])
        val recid = counterRecids[segment]
        val count = stores[segment].get(recid, Serializer.LONG_PACKED)
            ?: throw org.beammapdb.DBException.DataCorruption("counter not found")
    ...
    override fun put(key: K?, value: V?): V? {
        ...
        if(checkHashAfterSerialization){
            ...
            val key2 = org.beammapdb.Utils.clone(key, keySerializer)
            if(hash(key2)!=hash){
    ...
    protected fun clear(notifyListeners:Int=1) {
        ...
        for(segment in 0 until segmentCount) {
            org.beammapdb.Utils.lockWrite(locks[segment]) {
                val indexTree = indexTrees[segment]
                val store = stores[segment]
    ...
    protected fun getprotected(hash:Int, key:K, updateQueue:Boolean):V?{
        ...
        if(CC.ASSERT) {
            if(updateQueue && expireGetQueues!=null)
                ...
                org.beammapdb.Utils.assertReadLock(locks[segment])
        }
        if(CC.PARANOID && hash!= hash(key))
            throw AssertionError()
    ...
    private fun getprotectedQueues(expireGetQueues: Array<org.beammapdb.QueueLong>, i: Int, leaf: Array<Any>, leafRecid: Long, segment: Int, store: org.beammapdb.Store): Array<Any> {
        if(CC.ASSERT)
            org.beammapdb.Utils.assertWriteLock(locks[segment])
    ...
    override fun isEmpty(): Boolean {
        for(segment in 0 until segmentCount) {
            org.beammapdb.Utils.lockRead(locks[segment]) {
    ...
    override val size: Int
        get() = org.beammapdb.Utils.roundDownToIntMAXVAL(sizeLong())
    ...
    protected fun expireQueueFor(segment:Int, expireId: Long): org.beammapdb.QueueLong {
        return when(expireId and 3){
            ...
            else -> throw org.beammapdb.DBException.DataCorruption("wrong queue")
    ...
    protected fun expireEvictSegment(segment:Int){
        ...
        for (q in arrayOf(expireGetQueues?.get(segment), expireUpdateQueues?.get(segment), expireCreateQueues?.get(segment))) {
            q?.takeUntil(org.beammapdb.QueueLongTakeUntil { nodeRecid, node ->
                var purged = false;
        ...
        if(expireCompactThreshold!=null){
            val store = stores[segment]
            if(store is org.beammapdb.StoreDirect){
    ...
    protected fun expireEvictEntry(segment:Int, leafRecid:Long, nodeRecid:Long){
        ...
        val leaf = stores[segment].get(leafRecid, leafSerializer)
                ?: throw org.beammapdb.DBException.DataCorruption("linked leaf not found")
        ...
        throw org.beammapdb.DBException.DataCorruption("nodeRecid not found in this leaf")
    }
    ...
    protected fun valueUnwrap(segment:Int, wrappedValue:Any):V{
        ...
        return stores[segment].get(wrappedValue as Long, valueSerializer)
                ?: throw org.beammapdb.DBException.DataCorruption("linked value not found")
    }
    ...
    override fun verify(){
        ...
        for(segment in 0 until segmentCount) {
            segmentRead(segment) {
                ...
                if(tree is org.beammapdb.Verifiable)
                    tree.verify()
                ...
                tree.forEachKeyValue { index, leafRecid ->
                    if(leafRecids.add(leafRecid).not())
                        throw org.beammapdb.DBException.DataCorruption("Leaf recid referenced more then once")

                    if(tree.get(index)!=leafRecid)
                        throw org.beammapdb.DBException.DataCorruption("IndexTree corrupted")
                    ...
                    for(i in 0 until leaf.size step 3){
                        ...
                        if(segment!=hashToSegment(hash))
                            throw org.beammapdb.DBException.DataCorruption("Hash To Segment")
                        if(index!=hashToIndex(hash))
                            throw org.beammapdb.DBException.DataCorruption("Hash To Index")
                        val value = valueUnwrap(segment, leaf[i+1])

                        val expireRecid = leaf[i+2]
                        if(expireEnabled.not() && expireRecid!=0L)
                            throw org.beammapdb.DBException.DataCorruption("Expire mismatch")
                        if(expireEnabled && expireRecid!=0L
                                && expireRecids.add(expireNodeRecidFor(expireRecid as Long)).not())
                            throw org.beammapdb.DBException.DataCorruption("Expire recid used multiple times")

                    }
                }

                fun queue(qq: Array<org.beammapdb.QueueLong>?){
                    if(qq==null)
                        return
                    val q = qq[segment]
                    q.verify()
                    ...
                    q.forEach { expireRecid, leafRecid, timestamp ->
                        if(leafRecids.contains(leafRecid).not())
                            throw org.beammapdb.DBException.DataCorruption("leafRecid referenced from Queue not part of Map")
                        val leaf = leafGet(stores[segment], leafRecid)
                        ...
                        if(!found)
                            throw org.beammapdb.DBException.DataCorruption("value from Queue not found in leaf $leafRecid " + Arrays.toString(leaf))

                        if(expireRecids.remove(expireRecid).not())
                            throw org.beammapdb.DBException.DataCorruption("expireRecid not part of IndexTree")
                    }
                }
                ...
                if(expireRecids.isEmpty.not())
                    throw org.beammapdb.DBException.DataCorruption("Some expireRecids are not in queues")
            }
        }
    }
    ...
    override fun close() {
        org.beammapdb.Utils.lockWriteAll(locks)
        try {
            closeable?.close()
        }finally{
            org.beammapdb.Utils.unlockWriteAll(locks)
        }
    }
    ...
    fun calculateCollisionSize():Pair<Long,Long>{
        ...
        for(segment in 0 until segmentCount) org.beammapdb.Utils.lockRead(locks[segment]) {
            indexTrees[segment].forEachValue { leafRecid ->
                val leaf = leafGet(stores[segment], leafRecid)
                size += leaf.size / 3
    ...
    protected fun leafGet(store: org.beammapdb.Store, leafRecid:Long):Array<Any>{
