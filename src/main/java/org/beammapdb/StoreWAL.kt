...
import org.beammapdb.volume.ReadOnlyVolume
import org.beammapdb.volume.SingleByteArrayVol
...
class StoreWAL(
    ...
    protected val headBytes = ByteArray(org.beammapdb.StoreDirectJava.HEAD_END.toInt())
    ...
    init{
        if(checksum)
            throw org.beammapdb.DBException.WrongConfiguration("StoreWAL does not support checksum yet") //TODO StoreWAL checksums
        org.beammapdb.Utils.lock(structuralLock) {
            if (!volumeExistsAtStart) {
                realVolume.ensureAvailable(org.beammapdb.CC.PAGE_SIZE)
                ...
                realVolume.putLong(org.beammapdb.StoreDirectJava.ZERO_PAGE_LINK, parity16Set(0L))
        ...
        org.beammapdb.Utils.lockWrite(locks[segment]) {
            if (di != null) {
                if (di.pos == 0) {
                    ...
                    val offset = getLong(di.buf, 0).ushr((7 - di.pos) * 8)
