...
class QueueLong(
    ...
    fun put(timestamp: Long, value:Long, nodeRecid:Long){
        ...
        if(prevRecid!=0L){
            val prevNode = store.get(prevRecid, Node.SERIALIZER)
                ?:throw org.beammapdb.DBException.DataCorruption("prev node not found")
    ...
    fun takeUntil(f: QueueLongTakeUntil){
        while(true){
            ...
            if(CC.ASSERT && node.prevRecid!=0L)
                throw org.beammapdb.DBException.DataCorruption("prevRecid not 0")
            ...
            if(CC.ASSERT && node.value!=nodeTaken!!.value)
                throw org.beammapdb.DBException.DataCorruption("wrong nodes")
        }
    }
    ...
    fun remove(nodeRecid: Long, removeNode:Boolean): Node {
        ...
        if(nextNode!=null) {
            if(CC.ASSERT && nextNode.prevRecid!=nodeRecid)
                throw org.beammapdb.DBException.DataCorruption("node link error")
            store.update(node.nextRecid, nextNode.copy(prevRecid = node.prevRecid), Node.SERIALIZER)
        }else{
            if(CC.ASSERT && headPrev!=nodeRecid)
                throw org.beammapdb.DBException.DataCorruption("headPrev error")
        ...
        if(node.prevRecid!=0L) {
            ...
            if(CC.ASSERT && tail!=nodeRecid)
                throw org.beammapdb.DBException.DataCorruption("tail error")
            tail = node.nextRecid
        }
        return node;
    }
    ...
    fun bump(nodeRecid: Long, newTimestamp:Long){
        ...
        if(headPrev2==nodeRecid){
            ...
            val node = store.get(nodeRecid, Node.SERIALIZER)
                ?: throw org.beammapdb.DBException.DataCorruption("link error")
    ...
    fun clear(){
        takeUntil(org.beammapdb.QueueLongTakeUntil { l, p -> true })
    }
    ...
    fun size():Long{
        ...
        while(head!=currentRecid){
            val node = store.get(currentRecid, Node.SERIALIZER)
                ?: throw org.beammapdb.DBException.DataCorruption("linked queue node not found")
