...
import org.beammapdb.BTreeMapJava.*
...
class BTreeMap<K,V>(
...
): org.beammapdb.Verifiable, Closeable, Serializable, org.beammapdb.ConcurrencyAware,
        ConcurrentNavigableMap<K, V>, org.beammapdb.ConcurrentNavigableMapExtra<K, V> {
    ...
    companion object {
        fun <K, V> make(
                ...
                store: org.beammapdb.Store = org.beammapdb.StoreTrivial(),
        ...
        internal fun <K, V> putEmptyRoot(store: org.beammapdb.Store, keySerializer: GroupSerializer<K>, valueSerializer: GroupSerializer<V>): Long {
            return store.put(
                    store.put(
                            org.beammapdb.BTreeMapJava.Node(LEFT + RIGHT, 0L, keySerializer.valueArrayEmpty(),
                                    valueSerializer.valueArrayEmpty()),
                            org.beammapdb.BTreeMapJava.NodeSerializer(keySerializer, keySerializer, valueSerializer)),
                    Serializer.RECID)
        }
    ...
    private val hasBinaryStore = store is org.beammapdb.StoreBinary

    protected val valueNodeSerializer = (if(valueInline) this.valueSerializer else Serializer.RECID) as GroupSerializer<Any>
    protected val nodeSerializer = org.beammapdb.BTreeMapJava.NodeSerializer(this.keySerializer, this.comparator, this.valueNodeSerializer);

    protected val rootRecid: Long
        get() = store.get(rootRecidRecid, Serializer.RECID)
                ?: throw org.beammapdb.DBException.DataCorruption("Root Recid not found");
    ...
    private fun getBinary(key: K): V? {
        val binary = store as org.beammapdb.StoreBinary

        var current = rootRecid

        val binaryGet = org.beammapdb.BTreeMapJava.BinaryGet<K, Any>(keySerializer, valueNodeSerializer, comparator, key)
    ...
    private fun isLinkValue(pos:Int, A: org.beammapdb.BTreeMapJava.Node):Boolean{
        ...
        try {
            ...
            do {
                    ...
                    if(!isLinkValue(pos, A)) {
                        ...
                        if (!onlyIfAbsent) {
                            if (valueInline) {
                                ...
                                A = org.beammapdb.BTreeMapJava.Node(flags, A.link, A.keys, values)
                ...
                if (!isRoot) {
                    ...
                    //is root
                    val R = org.beammapdb.BTreeMapJava.Node(
                            DIR + LEFT + RIGHT,
                            0L,
        ...
        try {
            ...
            if (pos >= 1 - A.intLeftEdge() && pos < keysSize - 1 + A.intRightEdge() + A.intLastKeyTwice()) {
                ...
                if (expectedOldValue == null || (oldValueExpanded!=null && valueSerializer.equals(expectedOldValue!!, oldValueExpanded))) {
                    ...
                    if(values!=null) {
                        A = org.beammapdb.BTreeMapJava.Node(flags, A.link, keys, values)
                        store.update(current, A, nodeSerializer)
                    }
    ...
    private fun copySplitLeft(a: org.beammapdb.BTreeMapJava.Node, splitPos: Int, link: Long): org.beammapdb.BTreeMapJava.Node {
        ...
        return org.beammapdb.BTreeMapJava.Node(flags, link, keys, values)

    }

    private fun copySplitRight(a: org.beammapdb.BTreeMapJava.Node, splitPos: Int): org.beammapdb.BTreeMapJava.Node {
        ...
        return org.beammapdb.BTreeMapJava.Node(flags, a.link, keys, values)
    }


    private fun copyAddKeyLeaf(a: org.beammapdb.BTreeMapJava.Node, insertPos: Int, key: K, value: V): org.beammapdb.BTreeMapJava.Node {
    ...
    private fun copyAddKeyDir(a: org.beammapdb.BTreeMapJava.Node, insertPos: Int, key: K, newChild: Long): org.beammapdb.BTreeMapJava.Node {
        ...
        return org.beammapdb.BTreeMapJava.Node(a.flags.toInt(), a.link, keys, values)
    }
    ...
    override fun verify() {
        fun verifyRecur(node: org.beammapdb.BTreeMapJava.Node, left: Boolean, right: Boolean, knownNodes: LongHashSet, nextNodeRecid: Long) {
...
//                throw AssertionError("right does not match $right")
    ...
    private fun getNode(nodeRecid: Long) =
            store.get(nodeRecid, nodeSerializer)
                    ?: throw org.beammapdb.DBException.DataCorruption("Node not found")

    private fun nodeToString(nodeRecid:Long?, node: org.beammapdb.BTreeMapJava.Node):String{
    ...
    private val descendingMap = org.beammapdb.BTreeMapJava.DescendingMap(this, null, true, null, false)
    ...
    override val keys: NavigableSet<K?> =
            org.beammapdb.BTreeMapJava.KeySet<K>(this as ConcurrentNavigableMap2<K, Any>, this.hasValues)
    ...
    abstract class BTreeIterator<K,V>(val m: BTreeMap<K, V>){
        ...
        protected var currentLeaf: org.beammapdb.BTreeMapJava.Node? = null
        ...
        private fun advanceFrom(recid: Long) {
            var node: org.beammapdb.BTreeMapJava.Node? =
                    if(recid==0L) null
        ...
        protected fun advance(){
            val currentLeaf: org.beammapdb.BTreeMapJava.Node = currentLeaf?:return
    ...
    fun descendingLeafIterator(hi: K?):Iterator<org.beammapdb.BTreeMapJava.Node>{
        ...
        return object : Iterator<org.beammapdb.BTreeMapJava.Node>{

            var nextNode: org.beammapdb.BTreeMapJava.Node? = null;
            ...
            override fun next(): org.beammapdb.BTreeMapJava.Node {
                val ret = nextNode
    ...
    fun prefixSubMap(prefix:K, inclusive:Boolean): ConcurrentNavigableMap<K, V> {
        ...
        return org.beammapdb.BTreeMapJava.SubMap(this, prefix, inclusive, hiKey, false)
    }
    ...
    override fun subMap(fromKey: K?,
        ...
        if (fromKey == null || toKey == null)
            throw NullPointerException()
        return org.beammapdb.BTreeMapJava.SubMap(this, fromKey, fromInclusive, toKey, toInclusive)
    }
    ...
    override fun headMap(toKey: K?,
        ...
        if (toKey == null)
            throw NullPointerException()
        return org.beammapdb.BTreeMapJava.SubMap(this, null, false, toKey, inclusive)
    }
    ...
    override fun tailMap(fromKey: K?,
        ...
        if (fromKey == null)
            throw NullPointerException()
        return org.beammapdb.BTreeMapJava.SubMap(this, fromKey, inclusive, null, false)
    }
