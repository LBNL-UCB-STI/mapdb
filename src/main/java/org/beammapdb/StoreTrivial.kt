...
open class StoreTrivial(
    ...
    protected val lock: ReadWriteLock? = org.beammapdb.Utils.newReadWriteLock(isThreadSafe)
    ...
    protected fun loadFromInternal(inStream: InputStream){
        ...
        org.beammapdb.Utils.logDebug { "Loaded ${records.size()} objects" }
    }
    ...
    protected fun fileHeaderCheck(header:Long){
        ...
        if(header.ushr(6*8) and 0xFF!= CC.FILE_TYPE_STORETRIVIAL)
            throw org.beammapdb.DBException.WrongFormat("Wrong file header, not StoreTrivial file")

        if(header.ushr(4*8) and 0xFFFF != 0L)
            throw org.beammapdb.DBException.NewMapDBFormat("Store was created with newer format, some new features are not supported")
    ...
    private fun preallocateInternal(): Long {
        ...
        if (old != null)
            throw org.beammapdb.DBException.DataCorruption("Old data were not null");
    ...
    override fun <R> get(recid: Long, serializer: Serializer<R>): R? {
        ...
        if(bytes===null){
            throw org.beammapdb.DBException.GetVoid(recid); //does not exist
        }

        if(bytes=== NULL_RECORD)
            return null;

        val dataIn = org.beammapdb.DataInput2.ByteArray(bytes)
...
class StoreTrivialTx(val file: File, isThreadSafe:Boolean=true, val deleteFilesAfterClose:Boolean=false)
    ...
    private val fileLock: FileLock =
                try {
                    ...
                    throw org.beammapdb.DBException.FileLocked(path!!, e)
                }
    ...
    protected fun loadFrom(number:Long){
        if(CC.ASSERT)
            org.beammapdb.Utils.assertWriteLock(lock)
        val readFrom = org.beammapdb.Utils.pathChangeSuffix(path, "." + number + DATA_SUFFIX)

        org.beammapdb.Utils.logDebug { "Loading from ${readFrom} with length ${readFrom.toFile().length()}" }
    ...
    override fun commit() {
        org.beammapdb.Utils.lockRead(lock) {
            ...
            Files.createFile(org.beammapdb.Utils.pathChangeSuffix(path, "." + next + COMMIT_MARKER_SUFFIX))
            lastFileNum = next
            //delete old data
            Files.deleteIfExists(org.beammapdb.Utils.pathChangeSuffix(path, "." + prev + COMMIT_MARKER_SUFFIX))
            Files.deleteIfExists(org.beammapdb.Utils.pathChangeSuffix(path, "." + prev + DATA_SUFFIX))
