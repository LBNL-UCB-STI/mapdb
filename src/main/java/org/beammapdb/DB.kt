...
import org.beammapdb.serializer.GroupSerializer
...
open class DB(
        ...
        private val store: org.beammapdb.Store,
                    ...
                    override fun run() {
                        for(o in shutdownHooks.keys.toTypedArray()) { //defensive copy, DB.close() modifies the set
                            try {
                                ...
                                org.beammapdb.Utils.LOG.log(Level.SEVERE, "DB.close() thrown exception in shutdown hook.", e)
                            }
                        }
                    }
                })
    ...
    fun getStore(): org.beammapdb.Store {
        checkNotClosed()
        return store
    }
...
//        val headInsertRecid = "#headInsertRecid"
    ...
    private fun namedClasses() = arrayOf(org.beammapdb.BTreeMap::class.java, org.beammapdb.HTreeMap::class.java,
            org.beammapdb.HTreeMap.KeySet::class.java,
            org.beammapdb.BTreeMapJava.KeySet::class.java,
            org.beammapdb.Atomic.Integer::class.java,
            org.beammapdb.Atomic.Long::class.java,
            org.beammapdb.Atomic.String::class.java,
            org.beammapdb.Atomic.Boolean::class.java,
            org.beammapdb.Atomic.Var::class.java,
            org.beammapdb.IndexTreeList::class.java
    )
    ...
    private val nameSer = object: ElsaSerializerBase.Ser<Any>(){
        override fun serialize(out: DataOutput, value: Any, objectStack: ElsaStack?) {
            val name = getNameForObject(value)
                    ?: throw org.beammapdb.DBException.SerializationError("Could not serialize named object, it was not instantiated by this db")
    ...
    private val elsaSerializer: ElsaSerializerPojo = ElsaSerializerPojo(
            ...
            object: ElsaClassInfoResolver {
                ...
                override fun getClassInfo(classId: Int): ClassInfo? {
    ...
    init{
        if(storeOpened.not()){
            ...
            if(store.isReadOnly){
                throw org.beammapdb.DBException.WrongConfiguration("Can not create new store in read-only mode")
            }
            ...
            if(CC.RECID_NAME_CATALOG != nameCatalogRecid)
                throw org.beammapdb.DBException.WrongConfiguration("Store does not support Reserved Recids: " + store.javaClass)
        ...
        if(!msgs.isEmpty())
            throw org.beammapdb.DBException.NewMapDBFormat("Name Catalog has some new unsupported features: " + msgs.toString());
    }
    ...
    init{
        ...
        val defSerName = "DB#defaultSerializer"
    ...
    init{
        if(shutdownReference!=null){
            addShutdownHook(shutdownReference)
        }
    }
    ...
    private fun loadClassInfos():Array<ClassInfo>{
        return store.get(org.beammapdb.CC.RECID_CLASS_INFOS, classInfoSerializer)!!
    }
    ...
    fun nameCatalogLoad(): SortedMap<String, String> {
        return org.beammapdb.Utils.lockRead(lock) {
            checkNotClosed()
    ...
    protected fun nameCatalogLoadLocked(): SortedMap<String, String> {
        if(CC.ASSERT)
            org.beammapdb.Utils.assertReadLock(lock)
        return store.get(org.beammapdb.CC.RECID_NAME_CATALOG, NAME_CATALOG_SERIALIZER)
                ?: throw org.beammapdb.DBException.WrongConfiguration("Could not open store, it has no Named Catalog");
    }

    fun nameCatalogSave(nameCatalog: SortedMap<String, String>) {
        org.beammapdb.Utils.lockWrite(lock) {
            checkNotClosed()
    ...
    protected fun nameCatalogSaveLocked(nameCatalog: SortedMap<String, String>) {
        if(CC.ASSERT)
            org.beammapdb.Utils.assertWriteLock(lock)
        store.update(org.beammapdb.CC.RECID_NAME_CATALOG, nameCatalog, NAME_CATALOG_SERIALIZER)
    }
    ...
    protected fun checkName(name: String) {
        if(name.contains('#'))
            throw org.beammapdb.DBException.WrongConfiguration("Name contains illegal character, '#' is not allowed.")
        if(!name.matches(nameRegex))
            throw org.beammapdb.DBException.WrongConfiguration("Name contains illegal character")
    }
    ...
    fun <E> nameCatalogGetClass(
        ...
        throw org.beammapdb.DBException.WrongConfiguration("Could not load object: " + clazz)
    }
    ...
    fun rollback(){
        if(store !is org.beammapdb.StoreTx)
    ...
    fun exists(name: String): Boolean {
        org.beammapdb.Utils.lockRead(lock) {
            checkNotClosed()
...
//    }
    ...
    class HashMapMaker<K,V>(
            ...
            protected val _storeFactory:(segment:Int)-> org.beammapdb.Store = { i-> db.store}
    ): Maker<HTreeMap<K, V>>(){
        ...
        fun layout(concurrency:Int, dirSize:Int, levels:Int): HashMapMaker<K, V> {
            fun toShift(value:Int):Int{
                return 31 - Integer.numberOfLeadingZeros(org.beammapdb.DataIO.nextPowTwo(Math.max(1,value)))
            }
        ...
        override fun verify(){
            if (_expireOverflow != null && _valueLoader != null)
                throw org.beammapdb.DBException.WrongConfiguration("ExpireOverflow and ValueLoader can not be used at the same time")
        ...
        override fun create2(catalog: SortedMap<String, String>): org.beammapdb.HTreeMap<K, V> {
            ...
            for (i in 0 until segmentCount) {
                val rootRecid = stores[i].put(org.beammapdb.IndexTreeListJava.dirEmpty(), IndexTreeListJava.dirSer)
            ...
            fun emptyLongQueue(segment: Int, qq: LongArrayList): org.beammapdb.QueueLong {
                val store = stores[segment]
                val q = store.put(null, org.beammapdb.QueueLong.Node.SERIALIZER);
                ...
                return org.beammapdb.QueueLong(store = store, tailRecid = tailRecid, headRecid = headRecid, headPrevRecid = headPrevRecid)
            }
            ...
            val indexTrees = Array<MutableLongLongMap>(1.shl(_concShift), { segment ->
                org.beammapdb.IndexTreeLongLongMap(
        ...
        override fun open2(catalog: SortedMap<String, String>): org.beammapdb.HTreeMap<K, V> {
            ...
            _keySerializer =
                    db.nameCatalogGetClass(catalog, name + if(hasValues) Keys.keySerializer else Keys.serializer)
                            ?: _keySerializer
            _valueSerializer = if(!hasValues) org.beammapdb.BTreeMap.Companion.NO_VAL_SERIALIZER as Serializer<V>
            else {
            ...
            fun queues(ttl: Long, queuesName: String): Array<org.beammapdb.QueueLong>? {
                if (ttl == 0L)
                    return null
                val rr = catalog[queuesName]!!.split(",").map { it.toLong() }.toLongArray()
                if (rr.size != segmentCount * 3)
                    throw org.beammapdb.DBException.WrongConfiguration("wrong segment count");
                return Array(segmentCount, { segment ->
                    org.beammapdb.QueueLong(store = stores[segment],
        ...
        override fun create(): org.beammapdb.HTreeMap<K, V> {
            return super.create()
        }

        override fun createOrOpen(): org.beammapdb.HTreeMap<K, V> {
            return super.createOrOpen()
        }

        override fun open(): org.beammapdb.HTreeMap<K, V> {
            return super.open()
        }
    }
    ...
    abstract class TreeMapSink<K,V>: org.beammapdb.Pump.Sink<Pair<K, V>, BTreeMap<K, V>>(){
    ...
    class TreeMapMaker<K,V>(
        ...
        private var _valueSerializer: GroupSerializer<V> =
                (if(hasValues) db.defaultSerializer else org.beammapdb.BTreeMap.Companion.NO_VAL_SERIALIZER) as GroupSerializer<V>
        ...
        fun <A> valueSerializer(valueSerializer: GroupSerializer<A>): TreeMapMaker<K, A> {
            if(!hasValues)
                throw org.beammapdb.DBException.WrongConfiguration("Set, no vals")
...
//        }
        ...
        fun createFromSink(): TreeMapSink<K, V> {
            ...
            val consumer = org.beammapdb.Pump.treeMap(
                    store = db.store,
                    keySerializer = _keySerializer,
                    valueSerializer = _valueSerializer,
        ...
        override fun create2(catalog: SortedMap<String, String>): BTreeMap<K, V> {
            ...
            val rootRecidRecid2 = _rootRecidRecid
                    ?: org.beammapdb.BTreeMap.Companion.putEmptyRoot(db.store, _keySerializer, _valueSerializer)
        ...
        override fun open2(catalog: SortedMap<String, String>): BTreeMap<K, V> {
            ...
            _valueSerializer =
                    if(!hasValues) {
                        org.beammapdb.BTreeMap.Companion.NO_VAL_SERIALIZER as GroupSerializer<V>
                    }else {
                        db.nameCatalogGetClass(catalog, name + Keys.valueSerializer) ?: _valueSerializer
                    }
            ...
            if(_valueSerializer!= org.beammapdb.BTreeMap.Companion.NO_VAL_SERIALIZER &&
    ...
    class HashSetMaker<E>(
        ...
        init{
            maker.valueSerializer(org.beammapdb.BTreeMap.Companion.NO_VAL_SERIALIZER).valueInline()
        }
        ...
        override fun open2(catalog: SortedMap<String, String>): org.beammapdb.HTreeMap.KeySet<E> {
            return maker.`%%%open2`(catalog).keys
        }

        override fun create2(catalog: SortedMap<String, String>): org.beammapdb.HTreeMap.KeySet<E> {
    ...
    abstract class Maker<E>(){
        ...
        protected fun make2(create:Boolean?):E{
            org.beammapdb.Utils.lockWrite(db.lock) {
                db.checkNotClosed()
                verify()
    ...
    class AtomicIntegerMaker(protected override val db: DB, protected override val name:String, protected val value:Int=0): Maker<Atomic.Integer>(){
        ...
        override fun create2(catalog: SortedMap<String, String>): org.beammapdb.Atomic.Integer {
            val recid = db.store.put(value, Serializer.INTEGER)
            catalog[name+ Keys.recid] = recid.toString()
            return org.beammapdb.Atomic.Integer(db.store, recid)
        }

        override fun open2(catalog: SortedMap<String, String>): org.beammapdb.Atomic.Integer {
    ...
    class AtomicLongMaker(protected override val db: DB, protected override val name:String, protected val value:Long=0): Maker<Atomic.Long>(){
        ...
        override fun create2(catalog: SortedMap<String, String>): org.beammapdb.Atomic.Long {
            val recid = db.store.put(value, Serializer.LONG)
            catalog[name+ Keys.recid] = recid.toString()
            return org.beammapdb.Atomic.Long(db.store, recid)
        }

        override fun open2(catalog: SortedMap<String, String>): org.beammapdb.Atomic.Long {
            val recid = catalog[name+ Keys.recid]!!.toLong()
            return org.beammapdb.Atomic.Long(db.store, recid)
        }
    }
    ...
    class AtomicBooleanMaker(protected override val db: DB, protected override val name:String, protected val value:Boolean=false): Maker<Atomic.Boolean>(){
        ...
        override fun create2(catalog: SortedMap<String, String>): org.beammapdb.Atomic.Boolean {
            val recid = db.store.put(value, Serializer.BOOLEAN)
            catalog[name+ Keys.recid] = recid.toString()
            return org.beammapdb.Atomic.Boolean(db.store, recid)
        }

        override fun open2(catalog: SortedMap<String, String>): org.beammapdb.Atomic.Boolean {
            val recid = catalog[name+ Keys.recid]!!.toLong()
            return org.beammapdb.Atomic.Boolean(db.store, recid)
        }
    }
    ...
    class AtomicStringMaker(protected override val db: DB, protected override val name:String, protected val value:String?=null): Maker<Atomic.String>(){
        ...
        override fun create2(catalog: SortedMap<String, String>): org.beammapdb.Atomic.String {
            val recid = db.store.put(value, Serializer.STRING_NOSIZE)
            catalog[name+ Keys.recid] = recid.toString()
            return org.beammapdb.Atomic.String(db.store, recid)
        }

        override fun open2(catalog: SortedMap<String, String>): org.beammapdb.Atomic.String {
            val recid = catalog[name+ Keys.recid]!!.toLong()
            return org.beammapdb.Atomic.String(db.store, recid)
        }
    }
    ...
    class AtomicVarMaker<E>(protected override val db: DB,
        ...
        override fun create2(catalog: SortedMap<String, String>): org.beammapdb.Atomic.Var<E> {
            val recid = db.store.put(value, serializer)
            catalog[name+ Keys.recid] = recid.toString()
            db.nameCatalogPutClass(catalog, name+ Keys.serializer, serializer)

            return org.beammapdb.Atomic.Var(db.store, recid, serializer)
        }

        override fun open2(catalog: SortedMap<String, String>): org.beammapdb.Atomic.Var<E> {
            val recid = catalog[name+ Keys.recid]!!.toLong()
    ...
    class IndexTreeLongLongMapMaker(
        ...
        override fun create2(catalog: SortedMap<String, String>): org.beammapdb.IndexTreeLongLongMap {
            catalog[name+ Keys.dirShift] = _dirShift.toString()
            catalog[name+ Keys.levels] = _levels.toString()
            catalog[name + Keys.removeCollapsesIndexTree] = _removeCollapsesIndexTree.toString()

            val rootRecid = db.store.put(org.beammapdb.IndexTreeListJava.dirEmpty(), IndexTreeListJava.dirSer)
            catalog[name+ Keys.rootRecid] = rootRecid.toString()
            return org.beammapdb.IndexTreeLongLongMap(
                    store = db.store,
                    rootRecid = rootRecid,
                    dirShift = _dirShift,
                    levels = _levels,
        ...
        override fun open2(catalog: SortedMap<String, String>): org.beammapdb.IndexTreeLongLongMap {
            return org.beammapdb.IndexTreeLongLongMap(
                    store = db.store,
    ...
    class IndexTreeListMaker<E>(
        ...
        override fun create2(catalog: SortedMap<String, String>): org.beammapdb.IndexTreeList<E> {
            catalog[name+ Keys.dirShift] = _dirShift.toString()
            catalog[name+ Keys.levels] = _levels.toString()
            catalog[name + Keys.removeCollapsesIndexTree] = _removeCollapsesIndexTree.toString()
            db.nameCatalogPutClass(catalog, name + Keys.serializer, serializer)
            ...
            val rootRecid = db.store.put(org.beammapdb.IndexTreeListJava.dirEmpty(), IndexTreeListJava.dirSer)
            catalog[name+ Keys.rootRecid] = rootRecid.toString()
            val map = org.beammapdb.IndexTreeLongLongMap(
                    store = db.store,
                    rootRecid = rootRecid,
                    dirShift = _dirShift,
                    levels = _levels,
            ...
            return org.beammapdb.IndexTreeList(
                    store = db.store,
                    map = map,
                    serializer = serializer,
                    isThreadSafe = db.isThreadSafe,
        ...
        override fun open2(catalog: SortedMap<String, String>): org.beammapdb.IndexTreeList<E> {
            val map = org.beammapdb.IndexTreeLongLongMap(
                    store = db.store,
                    dirShift = catalog[name + Keys.dirShift]!!.toInt(),
                    levels = catalog[name + Keys.levels]!!.toInt(),
    ...
    private fun defaultSerializerRegisterClass_noLock(clazz:Class<*>) {
        ...
        //and save
        store.update(org.beammapdb.CC.RECID_CLASS_INFOS, infos, classInfoSerializer)
    }
