...
import org.beammapdb.DataIO.parity1Get
import org.beammapdb.DataIO.parity1Set
import org.beammapdb.StoreDirectJava.RECID_LONG_STACK
import org.beammapdb.volume.Volume
import org.beammapdb.volume.VolumeFactory
...
abstract class StoreDirectAbstract(
    ...
    protected val structuralLock = org.beammapdb.Utils.newLock(isThreadSafe)
    protected val compactionLock = org.beammapdb.Utils.newReadWriteLock(isThreadSafe)
    ...
    protected var dataTail: Long
        get() = DataIO.parity4Get(headVol.getLong(org.beammapdb.StoreDirectJava.DATA_TAIL_OFFSET))
        set(v:Long){
            if(CC.ASSERT && (v%16)!=0L)
                throw org.beammapdb.DBException.DataCorruption("unaligned data tail")
            if(CC.ASSERT)
                org.beammapdb.Utils.assertLocked(structuralLock)
            headVol.putLong(org.beammapdb.StoreDirectJava.DATA_TAIL_OFFSET, DataIO.parity4Set(v))
        }

    /** maximal allocated recid */
    protected var maxRecid: Long
        get() = DataIO.parity3Get(headVol.getLong(org.beammapdb.StoreDirectJava.INDEX_TAIL_OFFSET)).ushr(3)
        set(v:Long){
            if(CC.ASSERT)
                org.beammapdb.Utils.assertLocked(structuralLock)
            headVol.putLong(org.beammapdb.StoreDirectJava.INDEX_TAIL_OFFSET, DataIO.parity3Set(v.shl(3)))
        }

    /** end of file (last allocated page) */
    //TODO add fileSize into Store interface, make this var protected
    internal var fileTail: Long
        get() = DataIO.parity16Get(headVol.getLong(org.beammapdb.StoreDirectJava.FILE_TAIL_OFFSET))
        set(v:Long){
            if(CC.ASSERT)
                org.beammapdb.Utils.assertLocked(structuralLock)
            headVol.putLong(org.beammapdb.StoreDirectJava.FILE_TAIL_OFFSET, DataIO.parity16Set(v))
        }

    protected fun fileHeaderCheck(){
        val header = headVol.getLong(0)
        if(header.ushr(7*8)!= CC.FILE_HEADER){
            throw org.beammapdb.DBException.WrongFormat("Wrong file header, not MapDB file")
        }
        if(header.ushr(6*8) and 0xFF!= CC.FILE_TYPE_STOREDIRECT)
            throw org.beammapdb.DBException.WrongFormat("Wrong file header, not StoreDirect file")

        if(header.ushr(4*8) and 0xFFFF != 0L){
            throw org.beammapdb.DBException.NewMapDBFormat("Store was created with newer version of MapDB, some new features are not supported")
        }
        ...
        if(headVol.getInt(20)!=calculateHeaderChecksum()) {
            ...
            if(checksumHeaderBypass)
                org.beammapdb.Utils.LOG.warning{msg}
            else
                throw org.beammapdb.DBException.DataCorruption(msg)
        }

        if(header.toInt().ushr(org.beammapdb.CC.FEAT_ENCRYPT_SHIFT) and CC.FEAT_ENCRYPT_MASK!=0)
            throw org.beammapdb.DBException.WrongConfiguration("Store is encrypted, but no encryption method was provided")

        //fails if checksum is enabled, but not in header
        val checksumFeature = header.toInt().ushr(org.beammapdb.CC.FEAT_CHECKSUM_SHIFT) and CC.FEAT_CHECKSUM_MASK
        if(checksumFeature==0 && checksum)
            throw org.beammapdb.DBException.WrongConfiguration("Store was created without checksum, but checksum is enabled in configuration")
        if(checksumFeature==1 && !checksum)
            throw org.beammapdb.DBException.WrongConfiguration("Store was created without checksum, but checksum is not enabled in configuration")
        if(checksumFeature>1){
            throw org.beammapdb.DBException.NewMapDBFormat("This version of MapDB does not support new checksum type used in store")
        }
        if(checksumFeature!=0 && this is org.beammapdb.StoreWAL)
            throw org.beammapdb.DBException.WrongConfiguration("StoreWAL does not support checksum")
        val checksumFromHeader = headVol.getLong(8)
        if(checksum){
            if(calculateChecksum()!=checksumFromHeader)
                throw org.beammapdb.DBException.DataCorruption("Wrong checksum in header")
        }else{
            if(1L!=checksumFromHeader)
                throw org.beammapdb.DBException.DataCorruption("Checksum is disabled, expected 1, got something else")
        }

        val featBits = headVol.getInt(4)
        if(featBits.ushr(3)!=0)
            throw org.beammapdb.DBException.NewMapDBFormat("Header indicates feature not supported in older version of MapDB")
        val storeFeatBits = headVol.getInt(16)
        if(storeFeatBits.ushr(1)!=0)
            throw org.beammapdb.DBException.NewMapDBFormat("Store header indicates feature not supported in older version of MapDB")

        if(storeFeatBits and 1 ==0 && checksumHeader)
            throw org.beammapdb.DBException.WrongConfiguration("Store header checksum, disabled in store, but enabled in configuration")

        if(storeFeatBits and 1 ==1 && !checksumHeader)
            throw org.beammapdb.DBException.WrongConfiguration("Store header checksum enabled in store, but disabled in configuration")
    }

    protected fun fileHeaderCompose():Long{
        val checksumFlag: Long = if(checksum)1L.shl(org.beammapdb.CC.FEAT_CHECKSUM_SHIFT) else 0
    ...
    protected fun loadIndexPages(indexPages: MutableLongList){
        ...
        while (true) {
            ...
            if (CC.ASSERT && nextPage % CC.PAGE_SIZE != 0L)
                throw org.beammapdb.DBException.DataCorruption("wrong page pointer")
    ...
    protected fun indexValCompose(size:Long,
        ...
        if(CC.ASSERT && (offset%16) != 0L)
            throw org.beammapdb.DBException.DataCorruption("unaligned offset")
    ...
    protected fun <R> deserialize(serializer: Serializer<R>, di: DataInput2, size: Long): R? {
        try{
            ...
            throw org.beammapdb.DBException.SerializationError(e)
        }
    }
    ...
    protected fun allocateData(size:Int, recursive:Boolean):Long{
        ...
        if(reusedDataOffset!=0L){
            ...
            if(CC.ASSERT && reusedDataOffset%16!=0L)
                throw org.beammapdb.DBException.DataCorruption("wrong offset")
