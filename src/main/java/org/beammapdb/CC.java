...
public interface CC{
    ...
    /** second byte in {@link StoreDirect} file format */
    long FILE_TYPE_STOREDIRECT  = 3;

    /** second byte in {@link StoreWAL} write ahead log */
    long FILE_TYPE_STOREWAL_WAL = 2;

    /** second byte in {@link SortedTableMap} file format,  with only single table (is probably read only)*/
    long FILE_TYPE_SORTED_SINGLE = 10;

    /** second byte in {@link SortedTableMap} file format, with multiple tables (is probably writeable)*/
    long FILE_TYPE_SORTED_MULTI = 11;

    /** second byte in {@link SortedTableMap} Write Ahead Log*/
    long FILE_TYPE_SORTED_WAL = 12;

    /** second byte in {@link StoreTrivial} file format */
