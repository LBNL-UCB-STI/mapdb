...
class SortedTableMapTest{
    ...
    @Test fun header(){
        ...
        assertEquals(org.mapdb.CC.FILE_HEADER, volume.getUnsignedByte(0).toLong())
        assertEquals(org.mapdb.CC.FILE_TYPE_SORTED_SINGLE, volume.getUnsignedByte(1).toLong())
    }
    ...
    fun issue695(){
        ...
        val sink = org.mapdb.SortedTableMap.Companion.create(
                volume,
    ...
    @Test fun headers2(){
        ...
        val s =  org.mapdb.SortedTableMap.Companion.create(vol, Serializer.LONG, Serializer.LONG).createFrom(HashMap())
        s.close()
        val raf = RandomAccessFile(f.path, "rw");
        raf.seek(0)
        assertEquals(org.mapdb.CC.FILE_HEADER.toInt(), raf.readUnsignedByte())
        assertEquals(org.mapdb.CC.FILE_TYPE_SORTED_SINGLE.toInt(), raf.readUnsignedByte())
        ...
        org.mapdb.TT.assertFailsWith(org.mapdb.DBException.NewMapDBFormat::class.java) {
            val vol = RandomAccessFileVol.FACTORY.makeVolume(f.path, false)
            val s = org.mapdb.SortedTableMap.Companion.open(vol, Serializer.LONG, Serializer.LONG)
        }
        f.delete()
    }
