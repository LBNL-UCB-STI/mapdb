...
class BTreeMap_PumpTest(
        val mapMaker: org.mapdb.DB.TreeMapMaker<Any?, Any?>
) {
        ...
        fun params(): Iterable<Any> {
            ...
            for(counter in bools){
                val db: DB = when(storeType){
                    0-> {
                        val d = org.mapdb.DBMaker.heapDB()
                        if(!isThreadSafe) d.concurrencyDisable()
                        d.make()
                    }
                    1-> DB(org.mapdb.StoreTrivial(), isThreadSafe = isThreadSafe, storeOpened = false)
                    2-> {
                        val d = org.mapdb.DBMaker.memoryDB()
                        if(!isThreadSafe) d.concurrencyDisable()
                        d.make()
                    }
    ...
    @Test fun test(){
        val limit = (100 + org.mapdb.TT.testScale() *1e6).toInt()
