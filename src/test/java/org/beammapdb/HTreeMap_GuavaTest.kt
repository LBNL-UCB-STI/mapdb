...
import org.beammapdb.guavaTests.ConcurrentMapInterfaceTest
...
class HTreeMap_GuavaTest(val mapMaker:(generic:Boolean)-> ConcurrentMap<Any?, Any?>) :
    ...
    companion object {
        ...
        fun params(): Iterable<Any> {
            ...
            val bools = if(org.mapdb.TT.shortTest()) org.mapdb.TT.boolsFalse else org.mapdb.TT.bools
            ...
            for(collapse in bools)
            {
                ret.add(arrayOf<Any>({generic:Boolean->
                    ...
                    var maker =
                            if(segmented) {
                                if(onHeap) org.mapdb.DBMaker.heapShardedHashMap(8)
                                else org.mapdb.DBMaker.memoryShardedHashMap(8)
                            }else {
                                val db =
                                        if(onHeap) org.mapdb.DBMaker.heapDB().make()
                                        else org.mapdb.DBMaker.memoryDB().make()
                                db.hashMap("aa")
                            }
                    ...
                    if(!generic)
                        maker.keySerializer(keySerializer).valueSerializer(org.mapdb.Serializer.STRING)
