...
public class DataIOTest {
    ...
    @Test public void parity1() {
        Assert.assertEquals(Long.parseLong("1", 2), DataIO.parity1Set(0));
        Assert.assertEquals(Long.parseLong("10", 2), DataIO.parity1Set(2));
        Assert.assertEquals(Long.parseLong("111", 2), DataIO.parity1Set(Long.parseLong("110", 2)));
        Assert.assertEquals(Long.parseLong("1110", 2), DataIO.parity1Set(Long.parseLong("1110", 2)));
        Assert.assertEquals(Long.parseLong("1011", 2), DataIO.parity1Set(Long.parseLong("1010", 2)));
        Assert.assertEquals(Long.parseLong("11111", 2), DataIO.parity1Set(Long.parseLong("11110", 2)));

        Assert.assertEquals(0, DataIO.parity1Get(Long.parseLong("1", 2)));
        try {
            DataIO.parity1Get(Long.parseLong("0", 2));
            fail();
        }catch(DBException.PointerChecksumBroken e){
            //TODO check mapdb specific error;
        }
        try {
            DataIO.parity1Get(Long.parseLong("110", 2));
            fail();
        }catch(DBException.PointerChecksumBroken e){
            //TODO check mapdb specific error;
        }
    ...
    @Test public void parityBasic(){
        for(long i=0;i<Integer.MAX_VALUE;i+= 1 + i/1000000L){
            if(i%2==0)
                Assert.assertEquals(i, DataIO.parity1Get(DataIO.parity1Set(i)));
            if(i%8==0)
                Assert.assertEquals(i, DataIO.parity3Get(DataIO.parity3Set(i)));
            if(i%16==0)
                Assert.assertEquals(i, DataIO.parity4Get(DataIO.parity4Set(i)));
            if((i&0xFFFF)==0)
                Assert.assertEquals(i, DataIO.parity16Get(DataIO.parity16Set(i)));
        }
    }

    @Test public void zeroParity(){
        assertTrue(DataIO.parity1Set(0)!=0);
        assertTrue(DataIO.parity3Set(0)!=0);
        assertTrue(DataIO.parity4Set(0)!=0);
        assertTrue(DataIO.parity16Set(0)!=0);
    }
    ...
    @Test public void shift(){
        for(int i =0; i<30;i++){
            ...
            assertEquals(i, DataIO.shift(DataIO.nextPowTwo(1<<i)));
            if(i>2)
                assertEquals(i, DataIO.shift(DataIO.nextPowTwo((1<<i)-1)));
        }
    }
