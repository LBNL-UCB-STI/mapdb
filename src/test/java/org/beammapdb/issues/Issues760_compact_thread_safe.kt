...
class Issues760_compact_thread_safe {
    ...
    fun compactShouldBeThreadSafe(tx:Boolean, withCompactThread:Boolean) {
        ...
        val db1 = org.mapdb.TT.tempFile()


        //val store = DBMaker.fileDB(db1).closeOnJvmShutdown().make().getStore()
        val store =  if(!tx) org.mapdb.StoreDirect.Companion.make(file = db1.absolutePath, volumeFactory = RandomAccessFileVol.FACTORY,
                fileLockWait = 0,
                allocateIncrement = 0,
                allocateStartSize = 0,
                isReadOnly = false,
                ...
                concShift = DataIO.shift(org.mapdb.DataIO.nextPowTwo(8)),
                checksum = false,
                isThreadSafe = true,
                checksumHeaderBypass = false)
            else
            org.mapdb.StoreWAL.Companion.make(file = db1.absolutePath, volumeFactory = RandomAccessFileVol.FACTORY,
                    fileLockWait = 0,
                    allocateIncrement = 0,
                    allocateStartSize = 0,
                    fileDeleteAfterClose = false,
    ...
    fun compactShouldBeThreadSafeWhenUsedByDB(tx:Boolean, withCompactThread:Boolean) {
        ...
        val make = if(!tx) org.mapdb.DBMaker.fileDB(db1).closeOnJvmShutdown().make()
                    else org.mapdb.DBMaker.fileDB(db1).closeOnJvmShutdown().transactionEnable().make()
        try {
