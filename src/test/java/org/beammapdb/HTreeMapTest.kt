...
class HTreeMapTest{
    ...
    @Test fun hashAssertion(){
        val map = org.mapdb.HTreeMap.Companion.make<ByteArray, Int>(keySerializer = Serializer.ELSA as Serializer<ByteArray>)

        try {
            for (i in 1..100)
                map.put(org.mapdb.TT.randomByteArray(10), 11)
            fail("hash exception expected")
        }catch(e:IllegalArgumentException){
            assertTrue(e.message!!.contains("hash"))
        }

        val map2 = org.mapdb.HTreeMap.Companion.make<Any, Int>(keySerializer = Serializer.ELSA,
                stores = arrayOf(org.mapdb.StoreOnHeap()), concShift = 0)
    ...
    @Test fun valueCreator(){
        val map = org.mapdb.HTreeMap.Companion.make<Int, Int>(valueLoader = { it + 10 })
    ...
    @Test fun close(){
        ...
        val map = org.mapdb.HTreeMap.Companion.make<Long, Long>(closeable = closeable)
        assertFalse(closed)
        map.close()
        assertTrue(closed)
    ...
    @Test fun test_hash_collision() {
        val m = org.mapdb.HTreeMap.Companion.make(keySerializer = HTreeMap_GuavaTest.singleHashSerializer, valueSerializer = Serializer.INTEGER, concShift = 0)
    ...
    @Test fun delete_removes_recids_dir_collapse(){
        ...
        val m = org.mapdb.HTreeMap.Companion.make(keySerializer = sequentialHashSerializer, valueSerializer = Serializer.INTEGER, concShift = 0)
    ...
    @Test fun clear(){
        val m = org.mapdb.HTreeMap.Companion.make(keySerializer = Serializer.INTEGER, valueSerializer = Serializer.INTEGER)
    ...
    @Test fun hasher() {
        val m = org.mapdb.DBMaker.memoryDB().make()
    ...
    @Test fun mod_listener_lock2() {
        ...
        m = db.hashMap("name", Serializer.STRING, Serializer.STRING)
            .modificationListener(org.mapdb.MapModificationListener { key, oldVal, newVal, triggered ->
...
//
    ...
    */
        ...
        m = db.hashMap("name", Serializer.STRING, Serializer.STRING)
                .modificationListener(object : MapModificationListener<String, String> {
                    override fun modify(key: String, oldValue: String?, newValue: String?, triggered: Boolean) {
                        ...
                        org.mapdb.Utils.assertWriteLock(m!!.locks[segment])
                        counter.incrementAndGet()
                    }
                })
                .create()
    ...
    @Test fun calculateCollisions(){
        val map = org.mapdb.DBMaker.heapDB().make().hashMap("name", Serializer.LONG, Serializer.LONG).createOrOpen()
