...
class VolumeSingleTest(val fab: Function1<String, Volume>) {
    ...
    companion object {
        ...
        fun params(): Iterable<Any> {
            ...
            for (o in org.mapdb.volume.VolumeTest.Companion.VOL_FABS) {
    ...
    @Test fun hash() {
        ...
        assertEquals(org.mapdb.CC.HASH_FACTORY.hash64().hash(b, 0, b.size, 11), v.hash(0, b.size.toLong(), 11))

        v.close()
    }

    @Test fun hashOverlap() {
        val b = ByteArray(org.mapdb.CC.PAGE_SIZE.toInt()*3)
        Random().nextBytes(b)
        val v = fab(org.mapdb.TT.tempFile().toString())
        v.ensureAvailable(org.mapdb.CC.PAGE_SIZE*4)
    ...
    @Test fun IOStreams(){
        val b = org.mapdb.TT.randomByteArray(1024 * 12 * 1024)
        val v = fab(org.mapdb.TT.tempFile().toString())
    ...
    internal fun putGetOverlap(vol: Volume, offset: Long, size: Int) {
        ...
        assertTrue(org.mapdb.Serializer.BYTE_ARRAY.equals(b, b2))
    }
    ...
    @Test fun copyFromDb(){
        ...
        val db = org.mapdb.DBMaker.volumeDB(volMem, false).make()
        val s = db.treeSet("aa", Serializer.STRING).createOrOpen()
        val s2 = TreeSet<String>()

        for(i in 0 until 10000){
            val a = org.mapdb.TT.randomString(10)
            s+=(a)
            s2+=a
        }
        db.commit()
        ...
        volMem.copyTo(vol2)
        val db2 = org.mapdb.DBMaker.volumeDB(vol2, true).make()
