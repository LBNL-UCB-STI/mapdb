...
class BTreeMap_ConcurrentMap_GuavaTest(
            ...
            for(counter in bools)
            {
                ret.add(arrayOf<Any>({generic:Boolean?->
                    val store = when(storeType){
                        0-> StoreOnHeap()
                        1-> StoreTrivial()
                        2-> org.mapdb.StoreDirect.Companion.make()
                    ...
                    org.mapdb.BTreeMap.Companion.make(keySerializer = keySer, valueSerializer = valSer,
    ...
    @Test fun randomInsert(){
        ...
        val max = if(org.mapdb.TT.shortTest()) 100 else 10000
