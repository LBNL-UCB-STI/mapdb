...
class StoreDirect_LongStackAllocTest(
    ...
    @Test fun run(){
        ...
        val store = org.mapdb.StoreDirect.Companion.make(isThreadSafe = false, concShift = 0)
        ...
        for(i in 0 until size){
            ...
            if( i % data.period == 0) {
                ba = org.mapdb.TT.randomByteArray(data.periodSize, seed = r.nextInt())
            ...
            if(recids.isEmpty.not() && r.nextInt(1000)<1000*data.updateProb){
                ...
                org.mapdb.TT.assertAllZero(old)
