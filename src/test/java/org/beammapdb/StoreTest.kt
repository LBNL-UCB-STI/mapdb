...
abstract class StoreTest {
    ...
    @Test fun put_get_large() {
        val e = openStore()
        val b = org.mapdb.TT.randomByteArray(1000000)
        Random().nextBytes(b)
    ...
    fun large_record() {
        val e = openStore()
        val b = org.mapdb.TT.randomByteArray(100000)
        val recid = e.put(b, Serializer.BYTE_ARRAY_NOSIZE)
    ...
    @Test fun large_record_delete2(){
        ...
        val b = org.mapdb.TT.randomByteArray(200000)
        val recid1 = s.put(b, Serializer.BYTE_ARRAY_NOSIZE)
        s.verify()
        val b2 = org.mapdb.TT.randomByteArray(220000)
    ...
    @Test fun large_record_update(){
        ...
        var b = org.mapdb.TT.randomByteArray(200000)
        val recid1 = s.put(b, Serializer.BYTE_ARRAY_NOSIZE)
        s.verify()
        val b2 = org.mapdb.TT.randomByteArray(220000)
        val recid2 = s.put(b2, Serializer.BYTE_ARRAY_NOSIZE)
        ...
        b = org.mapdb.TT.randomByteArray(210000)
        s.update(recid1, b, Serializer.BYTE_ARRAY_NOSIZE);
        assertTrue(Arrays.equals(b, s.get(recid1, Serializer.BYTE_ARRAY_NOSIZE)))
        assertTrue(Arrays.equals(b2, s.get(recid2, Serializer.BYTE_ARRAY_NOSIZE)))
        s.verify()

        b = org.mapdb.TT.randomByteArray(28001)
        s.update(recid1, b, Serializer.BYTE_ARRAY_NOSIZE);
        assertTrue(Arrays.equals(b, s.get(recid1, Serializer.BYTE_ARRAY_NOSIZE)))
        assertTrue(Arrays.equals(b2, s.get(recid2, Serializer.BYTE_ARRAY_NOSIZE)))
        s.verify()
    ...
    @Test fun preallocate_get_update_delete_update_get() {
        ...
        val recid = e.preallocate()
        e.verify()
        assertNull(e.get(recid, org.mapdb.TT.Serializer_ILLEGAL_ACCESS))
    ...
    @Test fun empty_update_commit() {
        ...
        for (i in 0..9999) {
            val s = org.mapdb.TT.randomString(80000)
            e.update(recid, s, Serializer.STRING)
            assertEquals(s, e.get(recid, Serializer.STRING))
            e.commit()
            assertEquals(s, e.get(recid, Serializer.STRING))
    ...
    @Test fun delete_reuse() {
        for(size in 1 .. 20){
            val e = openStore()
            val recid = e.put(org.mapdb.TT.randomString(size), Serializer.STRING)
            e.delete(recid, Serializer.STRING)
            org.mapdb.TT.assertFailsWith(org.mapdb.DBException.GetVoid::class.java) {
                e.get(recid, TT.Serializer_ILLEGAL_ACCESS)
            }

            val recid2 = e.put(org.mapdb.TT.randomString(size), Serializer.STRING)
    ...
    @Test fun randomUpdates() {
        ...
        val s = openStore()
        val random = Random(1);
        val endTime = org.mapdb.TT.nowPlusMinutes(10.0)
        ...
        for (i in 0 until maxSize){
            ...
            val b = org.mapdb.TT.randomByteArray(size, random.nextInt())
    ...
    @Test fun concurrent_CAS(){
        ...
        val ntime = org.mapdb.TT.nowPlusMinutes(1.0)
    ...
    @Test fun varRecordSizeCompact(){
        ...
        while(size<maxStoreSize) {
            ...
            for (i in 0..maxCount) {
                val r = org.mapdb.TT.randomByteArray(size, seed = i)
            ...
            fun verify() {
               //verify recids
                recids.forEachWithIndex { recid, i ->
                    ...
                    assertTrue(Arrays.equals(r, org.mapdb.TT.randomByteArray(size, seed = i)))
                }
            }
            verify()
            store.compact()
