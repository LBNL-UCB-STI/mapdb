...
import java.io.File
import org.beammapdb.StoreDirectJava.*
...
class StoreDirectTest: StoreDirectAbstractTest(){
    ...
    override fun openStore(file: File): StoreDirect {
        return org.mapdb.StoreDirect.Companion.make(file.path)
    }

    override fun openStore(): StoreDirect {
        return org.mapdb.StoreDirect.Companion.make()
    }
    ...
    @Test fun linked_getSet(){
        fun test(size:Int) {
            val b = org.mapdb.TT.randomByteArray(size, 1)
            val s = openStore()
    ...
    @Test open fun no_head_checksum(){
        var store = org.mapdb.StoreDirect.Companion.make(checksumHeader = false)
        assertEquals(0, store.volume.getInt(16)) //features
        assertEquals(0, store.volume.getInt(20)) //checksum

        store = org.mapdb.StoreDirect.Companion.make(checksumHeader = true)
    ...
    @Test fun checksum(){
        ...
        val store = org.mapdb.StoreDirect.Companion.make(volumeFactory = VolumeFactory.wrap(vol, false), checksum = false)
        ...
        val i = vol.getInt(4)
        assertEquals(0, i.ushr(org.mapdb.CC.FEAT_CHECKSUM_SHIFT) and CC.FEAT_CHECKSUM_MASK)

        //store reopen should not fail
        org.mapdb.StoreDirect.Companion.make(volumeFactory = VolumeFactory.wrap(vol, true), checksum = false).close()
        ...
        org.mapdb.TT.assertFailsWith(org.mapdb.DBException.WrongConfiguration::class.java) {
            org.mapdb.StoreDirect.Companion.make(volumeFactory = VolumeFactory.wrap(vol, true), checksum = true).close()
        }

        //set non zero checksum, it should fail to reopen
        vol.putLong(8,11)
        org.mapdb.TT.assertFailsWith(org.mapdb.DBException.DataCorruption::class.java) {
            org.mapdb.StoreDirect.Companion.make(volumeFactory = VolumeFactory.wrap(vol, true), checksum = false)
        }
    ...
    @Test fun checksum_enable(){
        val vol = SingleByteArrayVol(1024*1024*2)
        val store = org.mapdb.StoreDirect.Companion.make(volumeFactory = VolumeFactory.wrap(vol, false), checksum = true)
        ...
        val i = vol.getInt(4)
        assertEquals(1, i.ushr(org.mapdb.CC.FEAT_CHECKSUM_SHIFT) and CC.FEAT_CHECKSUM_MASK)
        ...
        vol.putLong(8,0)
        org.mapdb.TT.assertFailsWith(org.mapdb.DBException.DataCorruption::class.java) {
            org.mapdb.StoreDirect.Companion.make(volumeFactory = VolumeFactory.wrap(vol, true), checksum = true)
        }
...
abstract class StoreDirectAbstractTest: org.mapdb.StoreReopenTest() {
    ...
    @Test fun init_values(){
        val s = openStore()
        assertEquals(org.mapdb.CC.PAGE_SIZE, s.fileTail)
        assertEquals(0L, s.maxRecid)
        assertEquals(0L, s.dataTail)
        assertEquals(org.mapdb.CC.PAGE_SIZE, s.volume.length())
    ...
    @Test fun recid2Offset() {
        ...
        e.volume.ensureAvailable(org.mapdb.CC.PAGE_SIZE * 12)
        e.indexPages.add(org.mapdb.CC.PAGE_SIZE * 3)
        e.indexPages.add(org.mapdb.CC.PAGE_SIZE * 6)
        e.indexPages.add(org.mapdb.CC.PAGE_SIZE * 11)
        ...
        b.set(org.mapdb.CC.PAGE_SIZE.toInt() * 3 + 16, CC.PAGE_SIZE.toInt() * 4)
        b.set(org.mapdb.CC.PAGE_SIZE.toInt() * 6 + 16, CC.PAGE_SIZE.toInt() * 7)
        b.set(org.mapdb.CC.PAGE_SIZE.toInt() * 11 + 16, CC.PAGE_SIZE.toInt() * 12)
    ...
    @Test fun allocate_index_page(){
        ...
        val s2 = org.mapdb.StoreDirect.Companion.make(volumeFactory = VolumeFactory.wrap(s.volume, true))
    ...
    @Test fun alloc_data(){
        ...
        for(offset in CC.PAGE_SIZE until CC.PAGE_SIZE*2 step 1024){
            ...
            assertEquals(org.mapdb.CC.PAGE_SIZE*2, s.fileTail)
        }
        assertEquals(org.mapdb.CC.PAGE_SIZE*2, s.fileTail)
    }
    ...
    @Test fun alloc_data_overflow(){
        ...
        //allocate smaller
        assertEquals(org.mapdb.CC.PAGE_SIZE*2-1024, s.allocateData(16, false))
        assertEquals(org.mapdb.CC.PAGE_SIZE*2-1024+16, s.dataTail)

        //allocate which will cause overflow
        assertEquals(org.mapdb.CC.PAGE_SIZE*2+LONG_STACK_PREF_SIZE, s.allocateData(1024,false))
        assertEquals(org.mapdb.CC.PAGE_SIZE*2+1024+LONG_STACK_PREF_SIZE, s.dataTail)
        assertEquals(org.mapdb.CC.PAGE_SIZE*3, s.fileTail)
    ...
    @Test fun longStack_putTake_many() {
        val max2 = 10000L
        val min2 = if(org.mapdb.TT.shortTest()) max2 else 1
        val s = openStore()
        s.structuralLock?.lock()
        for(a in 1 .. 10) {
    ...
    @Test fun compact(){
        ...
        val random = Random()
        for(i in 1..1000){
            val string = org.mapdb.TT.randomByteArray(size = random.nextInt(100000), seed = random.nextInt())
    ...
    @Test open fun delete_after_close(){
        ...
        val store = org.mapdb.StoreDirect.Companion.make(dir.path + "/aa", fileDeleteAfterClose = true)
        store.put(11, Serializer.INTEGER)
        store.commit()
        store.put(11, Serializer.INTEGER)
        store.commit()
    ...
    @Test fun firstSize(){
        ...
        assertEquals(org.mapdb.CC.PAGE_SIZE, store.volume.length())
    ...
    @Test fun small_ser_size(){
        ...
        for(size in 1 .. 20){
            ...
            var b = org.mapdb.TT.randomByteArray(size, 1)
            ...
            //update same size
            b = org.mapdb.TT.randomByteArray(size, 2)
            ...
            //CAS the same size
            val b2 = org.mapdb.TT.randomByteArray(size, 3)
            assertFalse(store.compareAndSwap(recid, b2, org.mapdb.TT.randomByteArray(2, 4), Serializer.BYTE_ARRAY_NOSIZE))
    ...
    @Test fun test_sizes(){
        ...
        val arrays = sizes.map{ org.mapdb.TT.randomByteArray(it) }
    ...
    @Test fun head_feat_bits(){
        ...
        for(bitPos in firstUnknownBit until 32) {
            ...
            r.close()
            org.mapdb.TT.assertFailsWith(org.mapdb.DBException.NewMapDBFormat::class.java) {
                openStore(file)
            }
    ...
    @Test fun header_checksum_bypass(){
        ...
        val store = org.mapdb.StoreDirect.Companion.make(volumeFactory = VolumeFactory.wrap(vol, false), checksumHeaderBypass = false)
        ...
        vol.putInt(20, 0)
        //and reopen
        org.mapdb.StoreDirect.Companion.make(volumeFactory = VolumeFactory.wrap(vol, true), checksumHeaderBypass = true)
        org.mapdb.TT.assertFailsWith(org.mapdb.DBException.DataCorruption::class.java) {
            org.mapdb.StoreDirect.Companion.make(volumeFactory = VolumeFactory.wrap(vol, true), checksumHeaderBypass = false)
        }
    }
    ...
    @Test fun fileLoad(){
        val f = org.mapdb.TT.tempFile()
        val store = org.mapdb.StoreDirect.Companion.make(file = f.path, volumeFactory = MappedFileVol.FACTORY)
