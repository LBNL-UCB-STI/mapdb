...
class BTreeMapTest {
    ...
    @Test fun node_search() {
        val node = org.mapdb.BTreeMapJava.Node(
                DIR + LEFT,
                60L,
                arrayOf(10, 20, 30, 40),
    ...
    @Test fun flags() {
        ...
        val node2 = org.mapdb.BTreeMapJava.Node(
                DIR,
                111L,
                arrayOf(1, 1),
                longArrayOf(1)
    ...
    @Test fun getRoot() {
        ...
        val map = org.mapdb.BTreeMap.Companion.make<Int, Int>()
    ...
    @Test fun getRootLink() {
        ...
        val node1 = org.mapdb.BTreeMapJava.Node(
    ...
    @Test fun getRootMultiLink() {
        val map = org.mapdb.BTreeMap.Companion.make<Int, Int>()
        val node3 = org.mapdb.BTreeMapJava.Node(
                RIGHT,
                0L,
                arrayOf(70, 80, 90),
                arrayOf(8, 9)
    ...
    @Test fun getTree() {
        ...
        val dir = org.mapdb.BTreeMapJava.Node(
                DIR + LEFT + RIGHT,
                0L,
                arrayOf(50, 70),
    ...
    @Test fun iterate() {
        ...
        org.mapdb.TT.assertFailsWith(NoSuchElementException::class.java) {
            iter.next()
        }
    ...
    @Test fun descending_leaf_iterator_singleNode() {
        ...
        val nodeRecid = map.store.put(
                org.mapdb.BTreeMapJava.Node(LEFT + RIGHT, 0, arrayOf(1), arrayOf(10)),
                map.nodeSerializer
        )
    ...
    @Test fun descending_leaf_iterator_large() {
        val map = org.mapdb.BTreeMap.Companion.make<Int, Int>(maxNodeSize = 6)
        for (i in 1..100)
            map.put(i, i)
    ...
    @Test
    fun issue695(){
        val sink = org.mapdb.DBMaker.memoryDB().make().treeMap("a",
                Serializer.BYTE_ARRAY,
                Serializer.STRING).createFromSink()
        org.mapdb.TT.assertFailsWith(org.mapdb.DBException.NotSorted::class.java) {
            for (key in 120L..131) {
    ...
    @Test fun external_value(){
        val b = org.mapdb.BTreeMap.Companion.make(
                keySerializer = Serializer.INTEGER,
    ...
    @Test fun issue_38() {
        val max = 100+50000 * org.mapdb.TT.testScale()
        val map = org.mapdb.DBMaker.memoryDB().make().treeMap("test").create() as MutableMap<Int, Array<String?>>
    ...
    @Test fun findSmaller() {
        ...
        val m = org.mapdb.DBMaker.memoryDB().make().treeMap("test").create() as NavigableMap<Int, String>
    ...
    @Test fun concurrent_last_key() {
        ...
        //fill
        val c = 1000000 * org.mapdb.TT.testScale()
        for (i in 0..c) {
            m.put(i, i)
        }
    ...
    @Test fun WriteDBInt_lastKey() {
        ...
        val db1 = org.mapdb.DBMaker.memoryDB().make()
    ...
    @Test fun randomStructuralCheck() {
        val r = Random()
        val map = org.mapdb.DBMaker.memoryDB().make().treeMap("aa")
                .keySerializer(org.mapdb.Serializer.INTEGER).valueSerializer(org.mapdb.Serializer.INTEGER).create()

        val max = 100000 * org.mapdb.TT.testScale()
    ...
    fun large_node_size() {
        ...
        for (i in intArrayOf(10, 200, 6000)) {
            ...
            var db = org.mapdb.DBMaker.fileDB(f).fileMmapEnableIfSupported().make()
            var m = db.treeMap("map").maxNodeSize(i)
                    .keySerializer(org.mapdb.Serializer.INTEGER)
                    .valueSerializer(org.mapdb.Serializer.INTEGER).create()
            ...
            db.close()
            db = org.mapdb.DBMaker.fileDB(f).deleteFilesAfterClose().fileMmapEnableIfSupported().make()
    ...
    @Test fun issue403_store_grows_with_values_outside_nodes() {
        val f = org.mapdb.TT.tempFile()
        val db = org.mapdb.DBMaker.fileDB(f).closeOnJvmShutdown().make()

        val id2entry = db.treeMap("id2entry")
                .valueSerializer(org.mapdb.Serializer.BYTE_ARRAY)
                .keySerializer(org.mapdb.Serializer.LONG).valuesOutsideNodesEnable()
                .create()

        val store = db.getStore() as StoreDirect
        var b = org.mapdb.TT.randomByteArray(10000)
        id2entry.put(11L, b)
        val size = store.getTotalSize() - store.calculateFreeSize()
        for (i in 0..99) {
            val b2 = org.mapdb.TT.randomByteArray(10000)
        ...
        for (i in 0..99) {
            val b2 = org.mapdb.TT.randomByteArray(10000)
            assertTrue((id2entry as Map<Long, ByteArray>).replace(11L, b, b2))
            b = b2
        }
        assertEquals(size, store.getTotalSize() - store.calculateFreeSize())
    ...
    @Test fun setLong() {
        val k = org.mapdb.DBMaker.heapDB().make().treeSet("test").create() as org.mapdb.BTreeMapJava.KeySet<Int>
        k.add(11)
    ...
    fun testNullKeyInsertion() {
        val map = org.mapdb.DBMaker.memoryDB().make().treeMap("map").create() as MutableMap<Any?,Any?>
    ...
    fun serialize_clone() {
        val m = org.mapdb.DBMaker.memoryDB().make().treeMap("map", Serializer.INTEGER, Serializer.INTEGER).create()
        for (i in 0..999) {
            m.put(i, i * 10)
        }

        val m2 = org.mapdb.TT.cloneJavaSerialization(m)
    ...
    fun serialize_set_clone() {
        val m = org.mapdb.DBMaker.memoryDB().make().treeSet("map", Serializer.INTEGER).open()
        for (i in 0..999) {
            m.add(i)
        }
