...
class WALCrashTest: CrashJVM(){
    ...
    override fun doInJVM(startSeed: Long, params: String) {
        ...
        while(true){
            seed++
            startSeed(seed)
            val bb = org.mapdb.TT.randomByteArray(31, seed.toInt())
    ...
    override fun verifySeed(startSeed: Long, endSeed: Long, params: String): Long {
        ...
        wal.replayWAL(object: org.mapdb.WriteAheadLog.WALReplay by WriteAheadLog.NOREPLAY{
        ...
        assertArrayEquals(org.mapdb.TT.randomByteArray(31, lastLong!!.toInt()), lastBB)
    ...
    @Test fun run(){
        if(org.mapdb.TT.shortTest())
            return
        org.mapdb.crash.CrashJVM.Companion.run(this)
    }
}