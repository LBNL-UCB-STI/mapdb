...
class WALStreamCrashTest: CrashJVM(){
    ...
    @Test fun run1(){
        if(org.mapdb.TT.shortTest())
            return
        org.mapdb.crash.CrashJVM.Companion.run(this, killDelay = 1000, params = "8")
    }

    @Test fun run2(){
        if(org.mapdb.TT.shortTest())
            return
        org.mapdb.crash.CrashJVM.Companion.run(this, killDelay = 1000, params = "100")
    }
    ...
    @Test fun run3(){
        if(org.mapdb.TT.shortTest())
            return
        org.mapdb.crash.CrashJVM.Companion.run(this, killDelay = 1000, params = "1000")
    }
}
