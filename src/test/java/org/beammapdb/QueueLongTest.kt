...
class QueueLongTest {
    val q = org.mapdb.QueueLong.Companion.make()
    fun node(recid: Long) = q.store.get(recid, org.mapdb.QueueLong.Node.SERIALIZER)!!
    ...
    @Test fun insert_take() {
        ...
        assertNull(q.store.get(q.head, org.mapdb.QueueLong.Node.SERIALIZER))
        ...
        val node = q.store.get(recid, org.mapdb.QueueLong.Node.SERIALIZER)!!
        assertEquals(org.mapdb.QueueLong.Node(prevRecid = 0, nextRecid = q.head, timestamp = 111L, value = 222L), node)

        assertEquals(node.nextRecid, q.head)
        assertNull(q.store.get(node.nextRecid, org.mapdb.QueueLong.Node.SERIALIZER))
    ...
    @Test fun takeUntil() {
        ...
        assertEquals(4 + 3, q.store.getAllRecids().asSequence().count())
        q.takeUntil(org.mapdb.QueueLongTakeUntil { nodeRecid, node ->
            q.verify()
            nodeRecid == recid1
        })
        assertEquals(4 + 2, q.store.getAllRecids().asSequence().count())
