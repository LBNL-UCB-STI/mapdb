...
class UtilsTest{
    ...
    @Test fun single_entry_lock(){
        val lock = org.mapdb.Utils.singleEntryLock()
        lock.lock()
        lock.unlock()

        lock.lock()
        org.mapdb.TT.assertFailsWith(IllegalMonitorStateException::class.java) {
            lock.lock()
        }
        lock.unlock()
        org.mapdb.TT.assertFailsWith(IllegalMonitorStateException::class.java) {
    ...
    @Test fun single_entry_read_write_lock(){
        val lock = org.mapdb.Utils.SingleEntryReadWriteLock().writeLock()
        lock.lock()
        lock.unlock()
    ...
    fun lockWriteAll(){
        ...
        val waitUntilFinish = org.mapdb.TT.async2 {
        ...
        while(!locked.get()){}
        org.mapdb.Utils.lockWriteAll(locks)
    ...
    fun lockReadAll(){
        ...
        org.mapdb.Utils.lockReadAll(locks)
